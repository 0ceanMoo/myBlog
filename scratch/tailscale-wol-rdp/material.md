...
|----------|----|------|-----------|-------------|
| Mac Studio | macOS | 自宅の操作端末（普段使い） | - | `100.xxx.xxx.xxx` |
| MacBook Pro | macOS | 外出先の操作端末 | - | `100.xxx.xxx.xxx` |
...
- `wake.sh` が存在すること：
  ...
  ```
  ※ `XX:XX:XX:XX:XX:XX` は Windows の有線NICのMACアドレス
...
100.119.171.112  windows11      xxxxxxxx@  windows  接続中
100.xxx.xxx.xxx  mac-studio     xxxxxxxx@  macOS    接続中
100.83.12.94     raspberrypi    xxxxxxxx@  linux    接続中（exit node）← 復帰済み
100.xxx.xxx.xxx  macbook-pro    xxxxxxxx@  macOS    offline（外出時に接続）
100.xxx.xxx.xxx  118-27-1-226   xxxxxxxx@  linux    idle（用途不明）
100.xxx.xxx.xxx  iphone-15      xxxxxxxx@  iOS      offline
```
