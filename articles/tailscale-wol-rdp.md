...
「外出中に自宅のWindowsが必要になったけど、電源が入っていない…」

そんな状況を解決するために、**スリープ中のWindowsを外出先から起こしてリモート操作できる環境**を作りました。
...
- **フェーズ1**：まず自宅LAN内で Wake-on-LAN + RDP を動かす
- **フェーズ2**：Tailscale を使って外出先から同じことをする
...
WindowsをWoLで起こすには**常時起動しているデバイス**が必要です。今回は Raspberry Pi がその役割を担います。
...
WoLマジックパケットを送るスクリプトを作成します。`XX:XX:XX:XX:XX:XX` の部分は先ほど確認したWindowsのMACアドレスに置き換えてください。
...
これが重要なポイントです。Windowsがスリープから復帰したとき、Tailscaleが自動的に接続されないとRDPできません。
...
フェーズ1・2を通じて、外出先からスリープ中の自宅Windowsをリモート操作できる環境が完成しました。

```
MacBook Pro（外出先）
  └─[Tailscale]─> Raspberry Pi
                    └─[WoL]─> Windows（起動）
  └─[Tailscale + RDP]─> Windowsのデスクトップ
```

**ポイントまとめ：**

- WoLはWindowsの**有線LAN接続**と**BIOSおよびNICドライバの設定**が必要
- Raspberry Piは**常時起動のWoL中継役**として活用
- Tailscaleは**インストールしてログインするだけ**で使えるVPN
- WindowsのTailscaleサービスが**スリープ復帰後に自動接続**されることが重要

一度環境を整えてしまえば、あとは `rwake` 1コマンドで自宅のPCを起動してリモート操作できます。出先でWindowsが必要になっても慌てなくて済むようになりました。
